...
interface ApprovalTimelineCardProps {
  ...
  isLoading: boolean;
  /** Pass the entity's current status string; if 'DONE' the workflow is complete. */
  entityStatus?: string;
...
export const ApprovalTimelineCard = ({
  ...
  const isDone = entityStatus === 'DONE';
  ...
  const getLatestApprovalForLine = (lineNumber: number, status?: string) => {
  ...
  // Suppress unused-variable lint for isDone — reserved for callers that conditionally hide workflow
  void isDone;
