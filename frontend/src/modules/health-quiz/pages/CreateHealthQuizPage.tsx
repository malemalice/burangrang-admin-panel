...
import { CreateQuizDTO, CreateQuizQuestionDTO } from '@/modules/quizzes/types/quiz.types';
...
const CreateHealthQuizPage = () => {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const { createQuiz } = useHealthQuizzes();
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      toast.success(data.isPublished ? 'Questionnaire created and published' : 'Questionnaire saved as draft');
      navigate(`/health-quizzes/${newQuiz.id}`);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        title="Create health questionnaire"
        subtitle="Define declaration questions (no LMS scoring)"
        actions={
              ...
              <CardContent className="pt-6">
                <div className="flex justify-end gap-4">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => navigate('/health-quizzes')}
...
export default CreateHealthQuizPage;
