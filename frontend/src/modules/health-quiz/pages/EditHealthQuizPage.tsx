...
import QuizForm from '@/modules/quizzes/components/QuizForm';
import { useHealthQuiz } from '../hooks/useHealthQuizzes';
import { UpdateQuizDTO, CreateQuizQuestionDTO } from '@/modules/quizzes/types/quiz.types';
...
const formSchema = z.object({
  ...
  isDefaultForHealthScreening: z.boolean().default(false),
  ...
  path: ['entityId'],
}).refine((data) => !data.isDefaultForHealthScreening || data.isPublished, {
  message: 'Publish the questionnaire to set it as the default screening template',
  path: ['isDefaultForHealthScreening'],
}).refine((data) => !data.isDefaultForHealthScreening || !data.entity, {
  message: 'Only standalone questionnaires can be the default screening template',
  path: ['isDefaultForHealthScreening'],
});

type FormValues = z.infer<typeof formSchema>;

const EditHealthQuizPage = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { quiz, isLoading, fetchQuiz } = useHealthQuiz(id || null);
  ...
  useEffect(() => {
    if (quiz) {
      form.reset({
        ...
        isDefaultForHealthScreening: quiz.isDefaultForHealthScreening ?? false,
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const quizData: UpdateQuizDTO = {
        ...
        isDefaultForHealthScreening: data.isDefaultForHealthScreening,
        questions: uploadedQuestions as CreateQuizQuestionDTO[],
      };

      await healthQuizService.updateQuiz(id, quizData);
      toast.success('Questionnaire updated');
      navigate(`/health-quizzes/${id}`);
    } catch (error) {
  ...
  if (!quiz) {
    return (
      <div className="text-center py-12">
        ...
        <Button onClick={() => navigate('/health-quizzes')}>
  ...
  return (
    <>
      <PageHeader
        title="Edit health questionnaire"
        subtitle={`Update "${quiz.title}"`}
        actions={
          <Button variant="outline" onClick={() => navigate(`/health-quizzes/${id}`)}>
              ...
              <CardContent className="pt-6">
                <div className="flex justify-end gap-4">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => navigate(`/health-quizzes/${id}`)}
                  >
                    Cancel
                  </Button>
                  <Button
...
export default EditHealthQuizPage;
