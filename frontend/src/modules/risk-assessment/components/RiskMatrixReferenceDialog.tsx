...
import riskMatrixService from '@/modules/risk-matrix/services/riskMatrixService';
import { RiskRatingEnum } from '@/modules/risk-matrix/types/risk-matrix.types';
...
interface RiskMatrixReferenceDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export const RiskMatrixReferenceDialog = ({ open, onOpenChange }: RiskMatrixReferenceDialogProps) => {
  ...
  const { likelihoodLevels, consequenceLevels, matrixMap, likelihoodNameMap, consequenceNameMap } =
    buildMatrixGrid();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Risk Matrix Reference</DialogTitle>
        </DialogHeader>
                ...
                <div className="border rounded-lg overflow-hidden">
                  <div className="bg-lime-500 p-3 text-center">
                    <h2 className="font-bold text-sm uppercase">
                      Probability and Consequences Classification Level
                    </h2>
                  </div>
                  <table className="w-full border-collapse">
                    <thead>
                      <tr className="bg-gray-200 dark:bg-gray-700">
                        <th className="border p-3 font-semibold text-sm text-center">
                          Probability Level
                        </th>
                        <th className="border p-3 font-semibold text-sm text-center bg-yellow-200 dark:bg-yellow-800">
                          Probability Level
                        </th>
                        <th className="border p-3 font-semibold text-sm text-center">
                          Consequences Level
                    ...
                    <tbody>
                      {Array.from({ length: Math.max(probabilities.length, consequences.length) }).map(
                        (_, index) => {
                          const probability = probabilities[index];
                          const consequence = consequences[index];
                          return (
                            <tr key={index} className="bg-yellow-50 dark:bg-yellow-950/30">
                              ...
                              <td className="border p-3 text-center bg-yellow-100 dark:bg-yellow-900/40 font-semibold">
