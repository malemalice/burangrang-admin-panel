...
export interface WaterQualityLabReportAggregateData {
  leftColumnLabels: readonly [string, string, string];
...
export function buildWaterQualityLabReportAggregate(
  ...
  const leftColumnLabels: [string, string, string] = [
    'Sample Period',
    'Treatment Plant',
    'Category',
  ];
  ...
  const parameterMap = new Map<
    string,
    { id: string; label: string; category: WaterQualityParameterCategoryEnum }
  >();
  ...
  for (const cat of PARAMETER_CATEGORY_ORDER) {
    for (const p of byCategory[cat]) {
      parameterColumns.push({ id: p.id, label: p.label, category: cat });
    }
  }
