...
export async function seedMasterApprovals(prisma: PrismaClient) {
  ...
  const dept3 = adminDepartment || departments[2] || departments[0]!;

  const pos1 = managerPosition || jobPositions.find((j) => j.level <= 5) || jobPositions[0]!;
  const pos2 = directorPosition || jobPositions.find((j) => j.level <= 3) || jobPositions[1] || jobPositions[0]!;
  const pos3 = headPosition || jobPositions.find((j) => j.level <= 2) || jobPositions[0]!;
  ...
  // Risk Assessment: 2-step approval (Manager → Director)
  ...
  console.log(`✅ Created Risk Assessment approval workflow (2 steps)`);
