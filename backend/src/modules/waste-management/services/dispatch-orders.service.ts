...
export class DispatchOrdersService {
  ...
  constructor(
    ...
    this.dispatchOrderMapper = this.dtoMapper.createMapper(DispatchOrderDto, {
      ...
      relations: {
        orderer: {
          mapper: (orderer) => orderer ? { id: orderer.id, firstName: orderer.firstName, lastName: orderer.lastName } : undefined,
        },
        creator: {
  ...
  async create(createDto: CreateDispatchOrderDto, userId: string): Promise<DispatchOrderDto> {
    const existing = await this.prisma.dispatchOrder.findUnique({ where: { dispatchCode: createDto.dispatchCode } });
    if (existing) {
      this.errorHandler.throwConflictCustom(`Dispatch Order with code ${createDto.dispatchCode} already exists`);
    }
  ...
  async update(id: string, updateDto: UpdateDispatchOrderDto): Promise<DispatchOrderDto> {
    const existing = await this.prisma.dispatchOrder.findUnique({ where: { id } });
    this.errorHandler.throwIfNotFoundById('Dispatch Order', id, existing);

    const data: any = { ...updateDto };
    if (updateDto.dispatchDate) data.dispatchDate = new Date(updateDto.dispatchDate);
