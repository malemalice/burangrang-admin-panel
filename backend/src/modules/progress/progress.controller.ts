...
export class ProgressController {
  ...
  @ApiResponse({ status: 200, type: ProgressDto, description: 'Chapter marked as complete successfully' })
  @ApiResponse({ status: 404, description: 'Chapter or enrollment not found' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden - user does not have access to this course' })
  ...
  @ApiResponse({
    ...
    description: 'Enrollment progress with chapter-by-chapter status retrieved successfully',
  })
  @ApiResponse({ status: 404, description: 'Enrollment not found' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden - cannot access other user\'s enrollment progress' })
  @Roles(Role.USER, Role.ADMIN, Role.SUPER_ADMIN)
  ...
  @ApiResponse({
    status: 200,
    type: [EnrollmentProgressDto],
    description: 'List of all enrollments with progress retrieved successfully',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @Roles(Role.USER, Role.ADMIN, Role.SUPER_ADMIN)
