...
- **Risk Assessment:** Create, list (paginated, filter by department, status, isActive, search), read, update, delete. Assessments have department, assessment date, creator, assignee, status (`SCHEDULED|DRAFT|OPEN|WAITING_APPROVAL|DONE|REJECTED`), action plan. Assessment creation accepts inline `items[]` array for bulk item creation. Items: risk, risk category, likelihood/consequence levels, risk matrix rating, interpretation, post-mitigation levels and rating. Each item may have a `RiskMitigationRecord` (eliminate/substitute/engineer/admin/PPE/transfer/accept/legalAspect controls). Approval workflow: submit for approval, approve/reject with notes, approval timeline card. Reminders auto-created daily for SCHEDULED assessments until assessment date. Zoho ticket sync on create and status change. PDF export client-side via `react-to-pdf`.
...
- **RiskAssessment (t_risk_assessment):** id, code, description, departmentId, assessmentDate, createdBy, status (`SCHEDULED|DRAFT|OPEN|WAITING_APPROVAL|DONE|REJECTED`), isActive, assigneeId?, actionPlan, deletedAt, deletedBy. Relations: department, creator, assignee, items (RiskAssessmentItem[]), zohoTicketMapping.
...
- [FR-2] Risk assessment status must follow the lifecycle: `SCHEDULED → DRAFT → OPEN → WAITING_APPROVAL → DONE/REJECTED`.
